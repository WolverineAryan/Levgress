...
const INITIAL_QUESTIONS = [
  ...
  },
  {
    skillName: "React",
    tier: "BASIC",
    question: "What is the function of the 'key' prop when rendering lists of elements in React?",
    options: [
      "To encrypt list items for secure data transfer",
      "To help React identify which items have changed, been added, or been removed",
      "To automatically style the elements with zebra striping",
      "To bind click event handlers to all items"
    ],
    answerIndex: 1
  },
  ...
  },
  {
    ...
    options: [
      "To define which components are allowed to render the effect",
      "To specify which values the effect relies on, re-running the effect only when these change",
      "To bind the effect to database columns",
      "To list HTML classes that trigger the effect"
    ],
    answerIndex: 1
  },
...
const seedQuestions = async () => {
  try {
    console.log('Connecting to database to seed questions...');
    await mongoose.connect(config.mongoUri);
    console.log('Database connected.');

    // Clear old questions
    console.log('Clearing old QuizQuestions...');
    await QuizQuestion.deleteMany();

    console.log(`Inserting ${INITIAL_QUESTIONS.length} initial quiz questions...`);
    await QuizQuestion.insertMany(INITIAL_QUESTIONS);
